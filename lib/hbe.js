...
(() => {
  ...
  const encryptedData = Uint8ArrayFromBase64(dataElement.innerText);
  const keySalt = Uint8ArrayFromBase64(dataElement.dataset['keysalt']);
  const iv = Uint8ArrayFromBase64(dataElement.dataset['ivsalt']);
  ...
  async function decrypt(decryptKey, iv) {
    const result = await crypto.subtle.decrypt({ 'name': 'AES-GCM', 'iv': iv, }, decryptKey, encryptedData);
